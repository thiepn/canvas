...
A complete Chromium/Firefox/WebKit run passed with:

- **61 passed**;
- **4 intentional skips**;
- **0 failures**.

The skips avoid redundantly running the ten-context load test in all three engines; the functional collaboration cases still run cross-browser.
...
- a client goes offline, editing visibly pauses, the peer edits, network returns, the client reconnects without page refresh, receives the missed edit, and can continue editing;
...
`scripts/audit-build.mjs` also rejects accidental test-bridge inclusion and records bundle measurements.
...
CI additionally records `npm audit --json` and rejects high/critical dependency findings.
...
A release commit must have a green quality workflow on its **exact SHA**. Older green runs do not certify later documentation/config/dependency changes. Deployment-specific checks remain explicitly separate from repository CI rather than being inferred from source design.
